...
func NewLendingMarketProposalCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Submit a proposal for the Tomahawk Lending Market along with an initial deposit.
Upon passing, the
...
func NewTreasuryProposalCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Args:  cobra.ExactArgs(1),
		Short: "Submit a proposal to the Tomahawk Treasury",
		Long: `Submit a proposal for the Tomahawk Treasury along with an initial deposit.
Upon passing, the
The proposal details must be supplied via a JSON file.`,
