...
import (
	...
	"github.com/tomahawk-network/tomahawk/v6/ibc"
	tomahawk "github.com/tomahawk-network/tomahawk/v6/types"
...
//  - sends over all tomahawk native tokens
...
func (k Keeper) OnRecvPacket(
	...
	// Get addresses in `tomahawk1` and the original bech32 format
	...
	// a sender's own account on tomahawk (sender == recipient)
	...
	if account != nil && tomahawk.IsSupportedKey(account.GetPubKey()) {
		return ack
	}
	...
	k.bankKeeper.IterateAccountBalances(ctx, recipient, func(coin sdk.Coin) (stop bool) {
		...
		err = k.transferKeeper.SendTransfer(
			...
			recipient,                 // recipient is the address in the tomahawk chain
...
// the IBC denomination, i.e port and channel on tomahawk for the voucher. It
// returns an error if:
