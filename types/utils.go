...
const (
	// MainnetChainID defines the tomahawk EIP155 chain ID for mainnet
	MainnetChainID = "tomahawk_9001"
	// TestnetChainID defines the tomahawk EIP155 chain ID for testnet
	TestnetChainID = "tomahawk_9000"
)

// IsMainnet returns true if the chain-id has the tomahawk mainnet EIP155 chain prefix.
func IsMainnet(chainID string) bool {
	return strings.HasPrefix(chainID, MainnetChainID)
}

// IsTestnet returns true if the chain-id has the tomahawk testnet EIP155 chain prefix.
...
// GettomahawkAddressFromBech32 returns the sdk.Account address of given address,
// while also changing bech32 human readable prefix (HRP) to the value set on
// the global sdk.Config (eg: `tomahawk`).
// The function fails if the provided bech32 address is invalid.
func GettomahawkAddressFromBech32(address string) (sdk.AccAddress, error) {
