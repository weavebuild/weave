...
## default port prefixes for tomad
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/tomad keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/tomad init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/tomad add-genesis-account \
    "$("$PWD"/build/tomad keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000atoma,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/tomad gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/tomad collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/tomad validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/tomad start --pruning=nothing --rpc.unsafe \
    ...
    tomahawk_PID=$!
    echo "started tomahawk node, pid=$tomahawk_PID"
    # add PID to array
    arr+=("$tomahawk_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test tomahawk node $HOST_RPC ..."
...
stop_func() {
    tomahawk_PID=$i
    echo "shutting down node, pid=$tomahawk_PID ..."

    # Shutdown tomahawk node
    kill -9 "$tomahawk_PID"
    wait "$tomahawk_PID"
