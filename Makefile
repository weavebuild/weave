...
LEDGER_ENABLED ?= true
BINDIR ?= $(GOPATH)/bin
TOMAHAWK_BINARY = tomad
TOMAHAWK_DIR = tomahawk
BUILDDIR ?= $(CURDIR)/build
SIMAPP = ./app
HTTPS_GIT := https://github.com/tomahawk-network/tomahawk.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=tomahawk \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(TOMAHAWK_BINARY) \
...
build-docker:
	...
	$(DOCKER) rm tomahawk || true
	# create a new container from the latest image
	$(DOCKER) create --name tomahawk -t -i ${DOCKER_IMAGE}:latest tomahawk
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp tomahawk:/usr/bin/tomad ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/tomahawk-network/tomahawk/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(TOMAHAWK_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(TOMAHAWK_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(TOMAHAWK_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(TOMAHAWK_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/tomahawk-network/tomahawk
.PHONY: format
...
ETHERMINT_URL      	= https://raw.githubusercontent.com/tomahawk-network/ethermint-v2/v0.10.0
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(TOMAHAWK_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\tomahawk\Z tomad/node "./tomad testnet --v 4 -o /tomahawk --keyring-backend=test --ip-addresses tomadnode0,tomadnode1,tomadnode2,tomadnode3"
	docker-compose up -d
else
	mkdir -p localnet-setup
	@$(MAKE) localnet-build

	if ! [ -f localnet-setup/node0/$(TOMAHAWK_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/localnet-setup:/tomahawk:Z tomad/node "./tomad testnet --v 4 -o /tomahawk --keyring-backend=test --ip-addresses tomadnode0,tomadnode1,tomadnode2,tomadnode3"; fi
	docker-compose up -d
endif
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\localnet-setup\node0\tomad:tomahawk\Z tomad/node "./tomad unsafe-reset-all --home=/tomahawk"
	@docker run --rm -v $(CURDIR)\localnet-setup\node1\tomad:tomahawk\Z tomad/node "./tomad unsafe-reset-all --home=/tomahawk"
	@docker run --rm -v $(CURDIR)\localnet-setup\node2\tomad:tomahawk\Z tomad/node "./tomad unsafe-reset-all --home=/tomahawk"
	@docker run --rm -v $(CURDIR)\localnet-setup\node3\tomad:tomahawk\Z tomad/node "./tomad unsafe-reset-all --home=/tomahawk"
else
	@docker run --rm -v $(CURDIR)/localnet-setup/node0/tomad:/tomahawk:Z tomad/node "./tomad unsafe-reset-all --home=/tomahawk"
	@docker run --rm -v $(CURDIR)/localnet-setup/node1/tomad:/tomahawk:Z tomad/node "./tomad unsafe-reset-all --home=/tomahawk"
	@docker run --rm -v $(CURDIR)/localnet-setup/node2/tomad:/tomahawk:Z tomad/node "./tomad unsafe-reset-all --home=/tomahawk"
	@docker run --rm -v $(CURDIR)/localnet-setup/node3/tomad:/tomahawk:Z tomad/node "./tomad unsafe-reset-all --home=/tomahawk"
endif
...
.PHONY: build-docker-local-tomahawk localnet-start localnet-stop
