...
func DefaultConfig() Config {
	...
	return Config{
		...
		ChainID:           fmt.Sprintf("tomahawk_%d-1", tmrand.Int63n(9999999999999)+1),
		NumValidators:     4,
...
// NewAppConstructor returns a new tomahawk AppConstructor
func NewAppConstructor(encodingCfg params.EncodingConfig) AppConstructor {
	return func(val Validator) servertypes.Application {
		return app.NewTomahawk(
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "tomad")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "tomacli")
